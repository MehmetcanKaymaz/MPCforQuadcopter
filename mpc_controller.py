...
class MPCController:
    def __init__(self, A=np.zeros((6, 6)), B=np.zeros((6, 3)), dt=0.01):
        ...
        self.Ad = Parameter((self.nx, self.nx), value=A)
        self.Bd = Parameter((self.nx, self.nu), value=B)
        self.objective = 0
    ...
    def __update_A_B(self, A, B):
        self.A = np.diag(np.ones(6))+A*self.dt
        self.B = B*self.dt
        self.Ad.value = self.A
        self.Bd.value = self.B

    def run_controller(self, A, B, x, xr):
        self.__update_A_B(A, B)
